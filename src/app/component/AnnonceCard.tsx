      ...
      >
        ...
        <div className="p-4 space-y-3">
          ...
          <div className="flex justify-between text-sm text-black">
            <span className="font-medium">{t("annonce.price")} :</span>
            <span>{annonce.price} MRU</span>
          </div>

          {/* Publié / Non publié */}
          <div className="flex justify-between items-center text-sm">
            <span className="font-medium text-black">{t("annonce.status")} :</span>
            {published ? (
          ...
          {/* Contact */}
          <div className="flex justify-between items-center text-sm">
            <span className="font-medium text-black">{t("annonce.contact")} :</span>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            >
              {loading
                ? t("common.loading")
                : published
                ? t("annonce.unpublishBtn")
                : t("annonce.publishBtn")}
            </button>

            <button
              onClick={(e) => {
                e.preventDefault();
                router.push(`/annonces/${annonce.id}/edit`);
              }}
              className="mt-2 px-3 py-1 text-xs font-medium rounded bg-blue-100 text-blue-700 hover:bg-blue-200"
            >
      ...
      {showConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white rounded-lg shadow-lg p-6 max-w-sm w-full">
            <h2 className="text-lg font-semibold mb-4 text-black">{t("annonce.confirmTitle") || "Confirmer la suppression"}</h2>
            <p className="text-sm text-gray-600 mb-6 text-black">
              {t("annonce.confirmDelete") }
            </p>
            <div className="flex justify-end gap-2">
              ...
              >
                {deleting ? t("common.loading") : t("common.delete") || "Supprimer"}
              </button>
            </div>
          </div>
        </div>
